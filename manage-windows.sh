#!/usr/bin/env bash
# ============================================================================
#  Morse Code Android · management console · WINDOWS
...
SRC_BOOK_DIR="${SRC_BOOK_DIR:-$HOME/koutoub/app-03-morse-code}"
KEYSTORE_DIR="${KEYSTORE_DIR:-$HOME/.keys}"
KEYSTORE_FILE="${KEYSTORE_FILE:-$KEYSTORE_DIR/wdiy-upload.keystore}"
APP_ID="${APP_ID:-org.workshopdiy.morsecode}"
AVD_NAME="${AVD_NAME:-morse_code_test}"
...
cmd_test() {
  ...
  local out="/tmp/morse-code-$(date +%Y%m%d-%H%M%S).png"
...
show_menu() {
  ...
  cat <<EOF
${BOLD}${MAGENTA}+==================================================================+
|   Morse Code Android . management console (Windows $SHELL_ENV)${RESET}${BOLD}${MAGENTA}
